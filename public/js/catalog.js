...
function loadCategory(categoryId) {
    ...
    categoryRequest = $.ajax({
        url: siteUrl + '/categories/' + getValue(categoryId),
        dataType: 'json',
        async: false,
        success: function(response) {
            if (response) {
...
function bindCategory(category) {
    categoryRequest = $.ajax({
        url: siteUrl + '/categories/' + category["id"] + '/content',
        dataType: 'html',
        async: true,
        success: function(response) {
            if (response) {
...
function loadCategoryObjects(categoryId) {
    objectRequest = $.ajax({
        async: false,
        url: siteUrl + '/objects/search?categoryid=' + categoryId + '&index=' + currentCategoryIndex,
        dataType: 'json',
        success: function(response) {
            if (response && response.length > 0) {
                if (response.length == 1 && currentCategoryIndex == 0) {
                    ...
                    $('#sideBar1 .search-results-pane .info-content .content .more-button').on('click', function() {
                        ...
                        objectRequest = $.ajax({
                            async: true,
                            url: siteUrl + '/objects/' + itemId + '/content',
                            dataType: 'html',
                            success: function(response) {
                    ...
                    $('#sideBar1 .search-results-pane-1 #mapFrame').html('<iframe src="'+ siteUrl + '/objects/map?categoryid=' + categoryId + '" width="1178" height="980" frameBorder="0" scrolling="no"></iframe>');
    ...
    objectRequest = $.ajax({
        ...
        success: function(response) {
            if (response) {
                ...
                objectRequest = $.ajax({
                    async: true,
                    url: siteUrl + '/objects/' + objectId + '/content',
                    dataType: 'html',
                    success: function(response) {
                        console.log(response);
                        if (response) {
